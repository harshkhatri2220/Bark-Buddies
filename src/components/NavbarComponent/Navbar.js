...
const Navbar = ({children}) => {
  ...
  const renderNavBar = !isSignupPage ? (
    <div className="flex z-50 justify-between items-center w-full bg-[#FFB267] shadow-lg  md:px-4 fixed text-black h-[3rem]  md:h-[3.5rem] ">
      ...
      {/* <div class="flex justify-center ml-auto mt-8">
        <div class="mb-3 xl:w-96">
          <div class="relative mb-4 flex w-full flex-wrap items-stretch ">
            <input
              type="search"
              class=" ml-[100px] hidden md:flex relative m-0  w-[1%] min-w-0 flex-auto rounded border border-solid border-neutral-300 bg-transparent text-white bg-clip-padding px-3 py-1.5 text-base font-normal  outline-none transition duration-300 ease-in-out focus:border-primary-600 focus:text-neutral-700 focus:shadow-te-primary focus:outline-none dark:border-neutral-600 dark:text-neutral-200 dark:placeholder:text-neutral-200 bg-white"
              placeholder="Search"
              aria-label="Search"
